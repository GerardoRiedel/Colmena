...
function crearHora($usuario, $hora, $idCiudad, $paciente, $prestador, $isapre, $confirmada, $observacion,$numerolicencia, $conectar)
{
	...
	VALUES
		($usuario, '$hora', '$hora_agendamiento', '$idCiudad',  $paciente, $prestador, $isapre, '$confirmada','$observacion','$numerolicencia')
	";
...
function eliminarHora($id, $usuario, $hora, $paciente, $prestador, $isapre, $conectar)
{
	...
	INSERT INTO 
		horas_eliminadas
		(usuario, fecha, idHora, hora, ciudad, licencia, idHoraPrestador, urlColmena, paciente, prestador, isapre)
	VALUES
		($usuario, now(), ".$id.", '$hora', ".$datosHora['ciudad'].", ".$datosHora['numeroLicenciaCheck'].", ".$datosHora['idHoraPrestador'].", ".$datosHora['urlExpedienteColmena'].", $paciente, $prestador, $isapre)
	", $conectar);	
	...
	$sqlupd= "DELETE from  horas  WHERE id=$id";
		$sqlu = mysql_query($sqlupd, $conectar);
	
	if($sqlu == false)
	{
		die('No se elimino la hora'.$sqlupd);
	}
}
...
function editarHora2($id, $isapre, $observacion,$numerolicencia, $conectar)
{
	mysql_query("
	UPDATE 
		horas 
...
function confirmarAsistenciaHora_old($id, $conectar)
{
	...
	$sql = mysql_query("SELECT h.`asiste` FROM horas h WHERE h.`id`=$id ", $conectar);
    ...
    while ($fila = mysql_fetch_array($result)) {
        ...
        $direccionperitaje = $fila['direccion'];
            ...
            $mensaje1 = "
                ...
                <p>Junto con  saludar, queremos recordar su citaci&oacute;n a peritaje psiqui&aacute;trico encargado por su  isapre <span class='negrita'>" . $isapre . " </span> para el <span class='negrita'> " . $fecha . " </span> a las " . $horacitacion . " en <span class='negrita'>" . $direccionperitaje . " </span>con la Dr(a). <span class='negrita'>" . $profesional . ".</span></p>
                <p><span class='negrita'><a href='" . $url . "'>Se adjunta mapa de la direcci&oacute;n:</a></span><br />
                </p>
                ...
                <p>
                  ...
                  " . $locomocion . "<br/>
                <span class='negrita'>�En qu� consiste esta citaci&oacute;n?</span>Se  trata de una entrevista o evaluaci&oacute;n de segunda opini&oacute;n con un psiquiatra externo a su  Seguro de Salud, quien lo evaluar� por su reposo laboral y emitir� un informe  t&eacute;cnico al respecto. </br/>
                Tenga presente que en esta evaluaci&oacute;n se le realizar&aacute;n a traves de un esquema estructurado, varias preguntas relacionadas con sus datos biogr&aacute;ficos y cl�nicos. Traer claramente detallados los nombres de los medicamentos que Ud. tiene indicados, con los miligramos y horario de ingesta.
                </p>
                
                <ul type= 'disc '>
                  <li>En esta entrevista de evaluaci&oacute;n, el psiquiatra deber� registrar la       informaci&oacute;n en un computador para hacer el informe por lo que habr�       momentos en que no lo mire a los ojos. Ello, no significa que no le est�       prestando atenci&oacute;n.</li>
                  ...
                  <li>En caso que llegue atrasado a la cita se le entregar� un       certificado indicando la hora de citaci&oacute;n y la hora de llegada, para que       Ud. solicite una nueva hora en la ISAPRE.  </li>
                </ul>
