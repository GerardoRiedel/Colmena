...
session_start();
	...
	if($idPrestador == 0)
	{
		...
		FROM 
			horas h
		WHERE 
			h.`hora` BETWEEN '$ano-$mes-01 00:00:00' AND '$ano-$mes-31 23:59:59' AND
			h.`isapre`=".isapreUsuario($_SESSION['idUsuario'], $conectar).$ciudadAdd."
		ORDER BY 
			h.`prestador` ASC, 
			h.`hora` ASC";
	}
...
<table width="878" height="57" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#000000" bgcolor="#FFFFFF" class="borde_tabla">
	<tr>
		...
		<td width="20%" align="center" class="letraDocumentoTitulo"><strong>Asiste</strong></td>
	</tr>
	<?php 
	$total = 0;
	...
	while($row = mysql_fetch_array($sql))
	{
		...
		<tr>
			...
			<td align="left"><?php echo $asiste; ?></td>
		</tr>
		<?php 
		$total++;
	}
