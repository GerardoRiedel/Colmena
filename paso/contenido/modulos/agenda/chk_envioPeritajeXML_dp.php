...
DATEDIFF(DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i:%s'),`i`.`hora`)>=3 AND (`i`.`numerolicencia` IS NOT NULL AND `i`.`numerolicencia` <> 0 ))AND i.`asiste`='si' ;";
...
while($row = mysql_fetch_array($result)) {
    ...
    $idpaciente = $row['paciente'];
        ...
        $file = crearInformePDFCB($idHora, $tipoSalida, $carpetaSalida, $conectar);
        $archivo =  $file;
        $TipoDocumento = $tipo;
...
// formateo de los datos de clase para envio en parametro webservice
    ...
    echo "$num_rows Informess de Peritaejes Enviados\n";
