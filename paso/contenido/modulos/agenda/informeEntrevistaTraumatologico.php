...
<!--FIN SCRIPTS-->
	<script type="text/javascript" src="<?php echo $LIB; ?>/jquery/11/jquery.min.js"></script>
...
function grabartraumatologia(){
	var data = $("#formtrauma").serialize();
	var msg = '';

	$.ajax({
		type: "POST",
		url: "chk_informeEntrevistaTraumatologicoNuevo.php",
		data: data,
		...
		//	msg = document.getElementById("resultadoAutoguardado");
			...
			if (data == 1) {//Bien
				...
				alertify
					.errorAlert("<h2>Datos actualizados a las  :"+ahora.getHours()+":"+ahora.getMinutes()+":"+ahora.getSeconds()+" </h2>");


			}else{//Mal


				alertify.error("registro NO Pudo ser Actualizado, espere unos segundos y vuelva a intentar");
...
<br />
<form id="formtrauma" name="formtrauma" method="post" action="">
			...
			<tr>
				...
				<span class="informeTituloCampo">Opini&oacute;n Sobre el Diagn&oacute;stico de Licencia M&eacute;dica N&deg; <?php echo $numeroLicencia; ?></span><br />
				  <select name="opinionSobreDiagnostico" id="opinionSobreDiagnostico">
				    ...
				    <option value="TIENE OTRO DIAGNOSTICO TRAUMATOLOGICO">TIENE OTRO DIAGNOSTICO TRAUMATOLOGICO</option>
				    <?php
						if($existe == true)
						{
							?>
	...
	<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
		<tr>
			<td align="center">
				...
				<label>
				<input name="grabar" type="button" class="boton" onclick="grabartraumatologia();" value="Guardar" />
				</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <?php
				if(tipoUsuario($_SESSION['idUsuario'], $conectar) != 'prestador')				
				{ ?>
