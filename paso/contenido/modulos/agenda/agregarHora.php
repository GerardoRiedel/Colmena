...
//        $usuario = usuarioHora($idHora, $conectar);
		...
		$fechaingresoLista =$registrosHora['fechaIngresoALista'] ;
...
<script>
            ...
            $.getJSON("<?php echo $LIB; ?>/querys/pacientesAutocompletar.php?rut=" + $("#rut").val(),
                function(data){
                    $.each(data, function(i,item){
                        ...
                        else if (item.field == "lic")
                            $("#numerolicencia").val(item.value);
    ...
    function reservarHora(){
			...
			var data = $("#formAgregarHora").serialize();
...
*/					//Si el usuario es una isapre puede ver el informe sólo si está úblicado.
	...
	{
		if(tipoUsuario($_SESSION['idUsuario'], $conectar) == 'administrador' || tipoUsuario($_SESSION['idUsuario'], $conectar) == 'secretaria' ||tipoUsuario($_SESSION['idUsuario'], $conectar) == 'isapre')
		{	
			$fecha = $_GET['fecha'];
			$idPrestador=$_GET['idPrestador'];
			$idCiudad=$_GET['idCiudad'];
			...
			<table width="90%" border="0" style="text-align:center" cellpadding="0" cellspacing="0">
				<tr>
					<td width="50%">
						<span class="letraDocumentoTitulo" style="cursor:pointer;" onclick="window.location.href='isapreHora.php?hora=<?php echo $idHoraPrestador; ?>'"> <img src="<?php echo $IMAGENES2; ?>/arrow_refresh.png" width="16" height="16" border="0"/> Vinculaci&oacute;n con Isapre </span>
					</td>
					<td width="50%">&nbsp;</td>
				</tr>
			</table>					
...
?>
		<br />
<!--form id="formAgregarHora" name="formAgregarHora" method="post"  action="chk_agregarHora.php"  -->
<form id="formAgregarHora" name="formAgregarHora" method="post"  action="">
...
?> onfocus=""/>
				...
				<label class="error nombre">Ingrese un nombre v&aacute;lido</label>
			</td>
		</tr>
		<tr>
...
?>/>
		...
		<tr>
			...
			<td class="letra7" style="padding-left:10px;">
				<label>
				<input name="email" type="text" id="email" size="30"  value="<?php echo $emailPaciente; ?>"/>
				</label>
			</td>
		</tr>
		<tr>
			...
			<td class="letra7" style="padding-left:10px;">
				<?php 
					if(tipoUsuario($_SESSION['idUsuario'], $conectar) != 'isapre')
					{
						...
						<option value="">Seleccione una Isapre</option>
							<?php 
							if($isapreHora != NULL)
							{
								?>
		...
		<tr>
			...
			<td class="letra7" style="padding-left:10px; padding-top:5px; padding-bottom:5px;">
				<label>
				<textarea name="observacion" cols="50" rows="4" class="letra7" id="observacion" ><?php echo $observacionHora; ?></textarea>
				</label>
			</td>
		</tr>
		<tr>
	...
	<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
		<tr>
			<td align="center">
                ...
                <div id="mensaje"  style="color: #F3565D"></div>
				<label>


                        <input id="btreserva" name="btreserva" type="submit" value="Reserva"  onmouseover="llamadaHP()" class="boton"   />

                    <!--input id="btmodifica" name="btmodifica" type="submit" value="Modifica" class="boton"  onmouseover="llamadaHP();" onclick="validar(form);" /-->
